...
import talib
import numpy as np
from chart_server.db.indicators import SuperTrend

SUPERTREND_TIMEPERIOD_1 = 10
SUPERTREND_MULTIPLIER_1 = 2
SUPERTREND_TIMEPERIOD_2 = 7
SUPERTREND_MULTIPLIER_2 = 3
chart_server = Flask(__name__)

minute_db = CLIENT['NSE_DB_MINUTE'].with_options(codec_options=CodecOptions(
    tz_aware=True, tzinfo=TIMEZONE), read_concern=pymongo.read_concern.ReadConcern('local'))
...
def get_formatted_ohlc(
    ...
    if backtest_stock is not None:
        ...
        start_date = dt.strptime(
            backtest_start_date, "%Y-%m-%d %H:%M:%S") - timedelta(days=3)
        end_date = dt.strptime(
            backtest_end_date, "%Y-%m-%d %H:%M:%S") + timedelta(days=5)
    elif request.method == 'POST':
        stock_name = request.form.get('stock-name')
        interval = request.form.get('select-interval')
        start_date = dt.strptime(request.form.get(
        ...
        start_date = (dt.now()-timedelta(days=14)).replace(hour=9,
    ...
    if interval == OHLCInterval.Tick:
        ...
        df = get_ohlc(stock_name, interval=interval,
        ...
        df['SuperTrend_2'] = SuperTrend(
            df['high'], df['low'], df['close'], timeperiod=SUPERTREND_TIMEPERIOD_2, multiplier=SUPERTREND_MULTIPLIER_2)[0]
...
def extract_trading_symbols():
    ...
    if ml:
        ...
        df = pd.read_csv(backtest_file_name, names=[
            'timestamp', 'tradingsymbol', 'entry_exit', 'buy_sell', 'qty', 'price', 'pnl', 'arg1'])
    # df = pd.read_csv(backtest_file_name)
    trading_symbols = []
...
def read_backtest():
    # temp modifier for testing ml backtest results
    ml = False
    ...
    if ml:
        df = pd.read_csv(backtest_file_name)
        print(df)
        df_buy = df[df['buy_sell'] == 1]
        df_sell = df[df['buy_sell'] == -1]
